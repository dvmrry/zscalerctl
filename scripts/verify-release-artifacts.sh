...
require_pattern 'cp LICENSE README\.md AGENTS\.md "dist/\$name/"' "release archives must include AGENTS.md for agent/operator guidance"
...
require_pattern 'cp man/zscalerctl\.1 "dist/\$name/man/"' "release archives must include man/zscalerctl.1"
require_pattern 'cp -R skills/zscalerctl "dist/\$name/skills/"' "release archives must include the zscalerctl skill"
