...
| Rendered and dumped output is limited to allow-listed fields, with secret-shaped values, bare high-entropy free-text tokens, and unmodeled nested values dropped or redacted. | `go test -mod=vendor ./internal/cli ./internal/zscaler` tests `TestResourceListProjectsAndRedactsFixture`, `TestResourceListRuleLabelsUsesCatalogProjection`, `TestDumpWritesRestrictedFilesAndReportsWithoutCanaries`, and `TestReaderListRuleLabelsProjectsSDKShapeThroughAllowList`. |
...
| Partial dumps cannot look complete and contain only value-free error metadata. | `go test -mod=vendor ./internal/cli` tests `TestDumpAbortsWithoutWritingOnResourceErrorByDefault` and `TestDumpContinueOnErrorWritesPartialManifestAndValueFreeErrors`. |
