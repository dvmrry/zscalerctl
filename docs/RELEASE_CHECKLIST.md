...
| `config show`, `doctor`, and `auth show` do not reveal credential values. | `go test -mod=vendor ./internal/cli` tests `TestConfigShowDoesNotExposeEnvironmentSecrets`, `TestDoctorDoesNotExposeEnvironmentSecrets`, and `TestAuthShowDoesNotExposeEnvironmentSecrets`. |
