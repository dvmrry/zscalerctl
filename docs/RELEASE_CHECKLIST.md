...
For product-specific branch validation, including the ZPA server-groups smoke,
follow [LIVE_SMOKE.md](LIVE_SMOKE.md). It includes fish-safe environment setup,
the no-prebuilt-binary path, the `--bin` build path, and the pass markers that
prove `--resources` was honored.
