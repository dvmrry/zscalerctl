...
| DLP and capture policy batch | `zia/dlp-engines`, `zia/dlp-dictionaries`, `zia/dlp-edm-schemas`, `zia/dlp-idm-profile-lite`, `zia/dlp-idm-profiles`, `zia/dlp-web-rules`, `zia/c2c-incident-receivers` | Cataloged with conservative field projection in a focused runtime-gated branch. Merge still requires focused smoke validation. |
...
| Policy/status metadata staging | `zia/ips-policies`, `zia/activation-status`, `zia/eusa-status`, `zia/auth-exempted-urls`, `zia/intermediate-ca-certificates` | Staged with conservative field projection. Merge requires focused runtime validation. IPS signature import/export and intermediate CA material/download operations remain excluded. |
| IPS signature-rule hold | `zia/ips-signature-rules` | SDK read path exists, but the package includes import, export, and validation operations. Probe endpoint availability before considering signature-rule surfaces. |
...
| `zia/c2c-incident-receivers` | Package `c2c_incident_receiver`; read functions `Get`, `GetC2CIRName`, `GetAllLite`, `GetAll`; endpoints `/zia/api/v1/cloudToCloudIR`, `/lite`, `/count`; `ValidateDelete` neighbor. | Cataloged with tenant authorization details and validation messages dropped. Merge requires focused runtime validation. |
| `zia/dlp-edm-schemas` | Package `dlp/dlp_exact_data_match`; read functions `GetDLPEDMSchemaID`, `GetDLPEDMByName`, `GetAll`; endpoint `/zia/api/v1/dlpExactDataMatchSchemas`; no mutating functions found in the package. | Cataloged with schema/file identifiers standard-only and token, column, admin, and schedule details dropped. Merge requires focused runtime validation. |
| `zia/dlp-idm-profile-lite` | Package `dlp/dlp_idm_profile_lite`; read functions `GetDLPProfileLiteID`, `GetDLPProfileLiteByName`, `GetAll`; endpoint `/zia/api/v1/idmprofile/lite`; no mutating functions found in the package. | Cataloged with profile and client identifiers standard-only; merge requires focused runtime validation. |
| `zia/dlp-idm-profiles` | Package `dlp/dlp_idm_profiles`; read functions `Get`, `GetByName`, `GetAll`; endpoint `/zia/api/v1/idmprofile`; no mutating functions found in the package. | Cataloged with host, path, account, schedule, and volume details standard-only and admin identity dropped. Merge requires focused runtime validation. |
| `zia/dlp-web-rules` | Package `dlp/dlp_web_rules`; read functions `Get`, `GetByName`, `GetAll`; endpoint `/zia/api/v1/webDlpRules`; create/update/delete neighbors. | Cataloged with criteria references standard-only and recursive sub-rules/admin identity dropped. Merge requires focused runtime validation. |
| `zia/ips-policies` | Package `ips_control_policies/ips_policies`; read functions `Get`, `GetByName`, `GetAll`; endpoint `/zia/api/v1/firewallIpsRules`; create/update/delete neighbors. | Staged as ordinary read-only policy metadata with IP/category selectors and nested associations limited to standard mode. Merge requires focused runtime validation. |
...
| `zia/intermediate-ca-certificates` | Package `intermediatecacertificates`; read functions `GetAll`, `GetCertificate`, `GetByName`; endpoint `/zia/api/v1/intermediateCaCertificate`; material/download and workflow neighbors include public-key, CSR, attestation, upload, and key-pair operations. | Staged only for certificate metadata. Public key material is dropped, CSR file names are standard-only, and material/download operations remain out of scope. Merge requires focused runtime validation. |
