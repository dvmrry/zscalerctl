...
- `answer` is **typed per question** by the question's declared `AnswerKind` (§2.2). Coercion is tolerant within a kind (`"12"`, `12`, `"twelve"` → 12 via the shared `coerceInt`). Scalar answers may be the scalar itself or an unambiguous structured wrapper; ambiguity is a miss, not a charitable parse.
...
Scalar answer kinds use kind-specific candidate extraction. A wrapper like `{"count":28,"product":"zpa"}` can satisfy a `count` question because there is exactly one integer candidate; `{"count":28,"other_possible_count":29}` fails because the grader cannot know which integer is the answer. The same rule applies to bools and enum strings. IDs are stricter: structured wrappers must expose an id-like field so record-shaped answers such as `{"id":"1","name":"HQ"}` do not make arbitrary string leaves eligible.
...
**Observed-command capture (the sidecar = the one observed-command stream):** the authoritative record of what the agent did is a single append-only stream at the **confined relative filename** in `ZSCALERCTL_FIXTURE_LOG` (read from each wrapper's own `os.Getenv`, so the path survives an agent's subprocess chain). The value is treated as a relative filename only — it is `filepath.Clean`ed and an absolute path or a `..` traversal is rejected (logging is silently skipped) so the env-controlled path can never write outside the working directory; it resolves against the process cwd, which the runner sets to the agent `WorkDir`. It is fed by **PATH-interposed logging wrappers**, not just the fixture binary:
